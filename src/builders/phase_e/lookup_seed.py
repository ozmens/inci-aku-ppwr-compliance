"""
Controlled lookup seeds for Phase E workbook platform.

Populates LKP_* only — no production masters (PRODUCT, COMPONENT, BOM, etc.).
CIP and CIF are both seeded as distinct Incoterms 2020 codes (not aliases).
"""
...
# Lookup-only seeds. Org / master / transactional tables stay empty.
LOOKUP_SEED_ROWS: dict[str, list[tuple]] = {
    "LKP_STATUS": [
    ...
    "LKP_LINE_ROLE": [
        ...
        (9, "LINER", "Liner"),
        (10, "LASHING", "Lashing"),
        (11, "OUTER_CARTON", "Outer carton"),
        (12, "UPPER_COVER", "Upper cover"),
        (13, "SEPARATOR", "Separator"),
        (14, "EDGE_PROTECTOR", "Edge protector"),
        (15, "SHRINK", "Shrink"),
        (16, "STRETCH", "Stretch"),
        (17, "STRAP", "Strap"),
        (18, "OTHER", "Other"),
    ...
    # CIP and CIF are distinct Incoterms 2020 terms — both canonical, neither an alias.
    "LKP_INCOTERM": [
        (1, "EXW", "Ex Works"),
        (2, "FCA", "Free Carrier"),
    ...
    "LKP_DOCUMENT_TYPE": [
        ...
        (8, "ARTWORK", "Artwork"),
        (9, "EVIDENCE", "Evidence"),
        (10, "SUPPLIER_DECLARATION", "Supplier Declaration"),
        (11, "LABEL", "Label"),
    ...
    for r_idx, row in enumerate(rows, start=2):
        for c_idx, value in enumerate(row, start=1):
            if c_idx <= len(headers):
                ...
                # Keep ID / code columns as text-capable
...
def apply_lookup_seeds(workbook: Workbook) -> list[str]:
    """Populate LKP_* sheets only. Returns seeded sheet names."""
    seeded: list[str] = []
    for sheet_name, rows in LOOKUP_SEED_ROWS.items():
...
def lookup_row_count() -> int:
    return sum(len(rows) for rows in LOOKUP_SEED_ROWS.values())
